  ...
  return (
    <div className=" ">
      <div className="grid grid-cols-3 gap-4 min-w-xl max-w-xl ">
        ...
        (<div key={index} className="rounded-[4px] overflow-hidden border border-gray-400" style={{ minWidth: '130px', minHeight: '200px', position: 'relative', margin: '0', scrollbarWidth: "none" }} >
          <div onClick={() => handlePinClick(pin.pin_url)} className="cursor-pointer rounded-[4px]" >
            <Image src={pin.image_url} alt={`Image ${index}`} layout="fill" objectFit="cover" className="post-media rounded-[4px]" />
          </div> </div>))} </div> </div>);
};
